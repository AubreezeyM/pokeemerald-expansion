...
#define P_UPDATED_TYPES             FALSE  // Since Gen 6, several Pokémon were changed to be partially or fully Fairy type.
...
#define P_UPDATED_EGG_GROUPS        FALSE  // Since Gen 8, certain Pokémon have gained new egg groups.
...
#define P_INCENSE_BREEDING              FALSE  // Since Gen 9, cross-generation Baby Pokémon don't require Incense being held by the parents to be obtained via breeding.
...
#define P_EGG_MOVE_TRANSFER             FALSE  // Starting in Gen 8, if two Pokémon of the same species are together in the Daycare, one knows an Egg Move, and the other has an empty slot, the other Pokémon will receive the Egg Move in the empty slot. In Gen 9, if a Pokémon holds a Mirror Herb, it will receive Egg Moves from the other regardless of species.
...
#define P_EV_CAP                    FALSE  // Since Gen 6, the max EVs per stat is 252 instead of 255.
